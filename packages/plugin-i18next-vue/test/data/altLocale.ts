...
export const getAltData = (options: {
  ...
  const AltLocales = Object.keys(neutral).reduce(
    (acc, key) => {
      ...
      acc[`alternative:foo.${neutral[key as keyof typeof neutral]}`] = `${locales[key as keyof typeof neutral]} 2`;
      return acc;
    },
    {} as {
      -readonly [key in `alternative:${(typeof neutral)[keyof typeof neutral]}`]: string;
    } & {
      -readonly [key in `alternative:foo.${keyof typeof neutral}`]: string;
    },
  );
  return {
