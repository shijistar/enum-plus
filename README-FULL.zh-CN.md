...
模板结果优先于原始声明的字符串，但不优先于函数：`name` 模板会覆盖 `options.name`，`items` 模板会覆盖枚举项元数据字段的原始值。原始声明的函数会直接返回最终的本地化结果，优先级最高，不会被模板覆盖。优先级顺序为：原始函数 > 实例模板 > 全局模板 > 原始字符串。如需引用原始值，请显式使用 `{name}` 和 `{raw}` 占位符。全局设置与实例配置可以并存：`Enum.config.templates` 对所有枚举生效，实例级 `templates` 按字段逐项合并，且实例配置优先级更高，覆盖同名字段的全局模板。
