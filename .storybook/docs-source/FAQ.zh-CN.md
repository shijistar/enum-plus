...
得益于 enum-plus 的 [插件系统](?path=/docs/recipes--docs&globals=locale:zh-CN#插件系统)，我们要为 enum-plus 扩展新功能变得非常容易。你可以根据功能的通用性和依赖关系，选择以下三种方式之一来贡献：
...
3. **自定义插件** - 新功能需要引入某个闭源的外部依赖，或者这个领域比较小众，建议你自己发布一个 npm 包，并把你的插件链接分享在 [插件生态](?path=/docs/recipes--docs&globals=locale:zh-CN#插件生态) 章节中，同样可以共享给所有人。

## 安全性

如果你发现安全问题，请遵循 [安全策略](https://github.com/shijistar/enum-plus/blob/master/SECURITY.md) 来负责任地报告。
