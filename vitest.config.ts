...
// @ts-expect-error: because alias should work in both nodejs and browser environments
const esLegacyIndex = fileURLToPath(new URL('./es-legacy/index.js', import.meta.url));
// @ts-expect-error: because alias should work in both nodejs and browser environments
const esLegacyDir = fileURLToPath(new URL('./es-legacy/', import.meta.url));
// @ts-expect-error: because alias should work in both nodejs and browser environments
...
export default defineConfig({
  test: {
    ...
    projects: [
      ...
      {
        // ---------- src, Browser Environment (browser-mode) ----------
        extends: true,
        define: {
      ...
      {
        // ---------- es-legacy, Browser Environment (browser-mode) ----------
        extends: true,
        define: {
          VITEST_ENGINE: '"vitest-browser"',
        },
        test: {
          name: 'es-legacy-browser',
          include: ['test/specs/vitest/**/*.vitest.ts'],
          setupFiles: [setupFile],
          browser: {
            enabled: true,
        ...
        resolve: {
          alias: [
            { find: '@enum-plus/', replacement: esLegacyDir },
            { find: '@enum-plus', replacement: esLegacyIndex },
          ],
        },
      },
    ],
